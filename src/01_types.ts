...
// 01 基础类型系统
{
  ...
  let f3: true = false // Error: 不能将类型 "false" 分配给类型 "true"
  ...
  let g4: 26.218 = 10 // Error: 不能将类型 10 分配给类型 26.218
  ...
  let e5: bigint = 100n // bigint
  let f5: 100n = 100n // bigint
  let g5: bigint = 100 // Error: 不能将类型 "number" 分配给类型 "bigint"
  ...
  let e6: string = 'zoom' // string
  let f6: 'john' = 'john' // 'john'
  let g6: 'john' = 'zoe' // Error: 不能将类型 "zoe" 分配给类型 "john"
  ...
  const f7: unique symbol = Symbol('f') // typeof f7
  let g7: unique symbol = Symbol('g') // Error: 类型为 "unique symbol" 的变量必须为 "const"。
  ...
  let i9: readonly number[] = [1, 2, 3] // readonly number[]
   ...
   * 元组支持剩余元素类型定义。
   */
  let a10: [number] = [1] // [number]

  let b10: [string, string , number] = ['a', 'b', 1]
  b10 = ['a', 'b', 1, 2] // Error: 不能将类型 "[string, string, number, number]" 分配给类型 "[string, string, number]"

  let c10: [number, number?][] = [
    [3.75],
    [8.25, 7.70],
    [10.50]
  ] // [number, number?][]

  let d10: [string, ...number[]] = ['a', 1, 2, 3]
  ...
  let myFirstLanguage: Language1 = Language1.English // 编译成赋值
  let mySecondLanguage: Language2 = Language2.Spanish // 编译成赋值
  ...
  let color1: Color1 = Color1.Green;  // 2
  ...
  let color2: string = Color2[2];  // 'Green'
}
